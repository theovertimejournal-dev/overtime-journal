...
def grade_game(game: dict, scores: dict) -> dict | None:
    ...
    # DEBUG — print all grading inputs so we can catch sign/direction bugs
    print(f"  [DEBUG] {matchup} | lean={lean} | lean_is_home={lean_is_home} | spread_home={game.get('spread_home')} | spread_away={game.get('spread_away')} | spread={spread} | margin={margin} | home_won={final['home_won']}")

    if spread is not None:
        try:
            spread_val = float(spread)
            # spread_home is negative when home is favored (BDL convention)
            # e.g. home fav by 5: spread_home = -5, spread_away = +5
            # e.g. home dog by 8: spread_home = +8, spread_away = -8
            # margin = home_score - away_score (positive = home won)
            #
            # If lean is home team:
            #   home covers if margin + spread_val > 0
            #   e.g. home -5 fav: spread_val=-5, home wins by 6: 6 + (-5) = +1 > 0 ✅
            #   e.g. home +8 dog: spread_val=+8, home loses by 3: -3 + 8 = +5 > 0 ✅
            # If lean is away team:
            #   away covers if (-margin) + spread_val > 0
            #   e.g. away -5 fav: spread_val (away) = -5, away wins by 6: margin=-6, -(-6)+(-5) = 1 > 0 ✅
            if lean_is_home:
                covered = (margin + spread_val) > 0
            else:
                covered = ((-margin) + spread_val) > 0
            result = "W" if covered else "L"
            print(f"  [DEBUG] spread grade: spread_val={spread_val} covered={covered} result={result}")
        except (ValueError, TypeError):
            result = "W" if (lean_is_home and final["home_won"]) or (not lean_is_home and not final["home_won"]) else "L"
            print(f"  [DEBUG] spread parse failed — falling back to ML grade: result={result}")
    else:
        # No spread — grade straight up (moneyline)
        result = "W" if (lean_is_home and final["home_won"]) or (not lean_is_home and not final["home_won"]) else "L"
        print(f"  [DEBUG] no spread — ML grade: result={result}")
