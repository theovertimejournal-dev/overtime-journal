...
def calculate_edge(home: dict, away: dict, spread_home=0) -> dict:
    ...
    FRANCHISE_STARS = {
        ...
        "Jayson Tatum":              ("BOS", 4.5),  # keep both Celtics stars
        ...
        if not hasattr(calculate_edge, '_ml_clf'):
            ...
            if os.path.exists(clf_path) and os.path.exists(feat_path):
                ...
                with open(clf_path, 'rb') as f:
                    calculate_edge._ml_clf = pickle.load(f)
    ...
    # ── Spread cap rules — data-driven from 1,043 game analysis ────────────
    # Finding: spreads above -12.5 cover below 52% (breakeven at -110).
    # At -15.5 cover rate drops to 39.7%. At -20.5 it's 27.4%.
    # Updated 2026-03-22: tightened from original 14.5/20 to 12.5/15.5
    spread_val = abs(float(spread_home or 0))

    if spread_val > 15.5:
        confidence = "INFO"   # Data says <40% cover rate — not a bet
    elif spread_val > 12.5 and confidence == "SHARP":
        confidence = "LEAN"   # Data says 52% cover — too thin for SHARP

    if spread_val > 12.5:
        signals.append({
            "type": "LARGE_SPREAD_CAUTION",
            "detail": f"Spread of {spread_val} pts — data shows {('<40%' if spread_val > 15.5 else '~52%')} cover rate at this level",
            "favors": "FADE",
            "strength": "CAUTION",
            "impact": -2.0,
        })
        ...
        if spread_val >= 6.0:
            ...
            if raw_net_diff >= 1.5:
                ...
                if gap >= 3.0:
                    ...
                    # ── TANK suppression: don't recommend spread value on tank teams ──
                    # Tank teams lose by 20-30 routinely. "Value" on their spread is a trap.
                    if dog_tier == "TANK":
                        dog_team = None  # suppress gut check entirely
                        print(f"  🚫 GUT CHECK SUPPRESSED — {dog_team} is TANK tier, spread 'value' is a trap", file=sys.stderr)
