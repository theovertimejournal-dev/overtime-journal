  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: user.avatar_color || "#ef4444",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 18, border: "2px solid rgba(255,255,255,0.08)",
      }}>
        {user.profile_icon || (user.username ? user.username[0].toUpperCase() : "?")}
      </div>
...
export default function LeaderboardPage({ currentUser }) {
  ...
  async function loadLeaderboard() {
    ...
    const { data: profiles } = await supabase
      .from('profiles')
      .select('user_id, username, avatar_color, profile_icon, bankroll, lifetime_earnings')
  ...
  async function handleSearch(q) {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('user_id, username, avatar_color, profile_icon')
      .ilike('username', `%${q}%`)
      .not('username', 'is', null)
      .limit(8);
