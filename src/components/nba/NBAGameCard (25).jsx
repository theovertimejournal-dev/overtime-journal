...
function BettingPanel({ game, user, profile, userPick, onPickPlaced, onChangePick }) {
  ...
  async function placeBet() {
    ...
    const { data, error: rpcErr } = await supabase.rpc('place_bet', {
      ...
      p_slate_date: game.date || new Date().toISOString().split('T')[0],
  ...
  if (userPick) {
    const won = userPick.result === 'win' || userPick.result === 'W';
    const lost = userPick.result === 'loss' || userPick.result === 'L';
    const pending = !won && !lost;
    const canChange = pending && !locked;
    const typeLabel = userPick.pick_type === 'moneyline' ? 'ML'
      : userPick.pick_type === 'spread' ? `Spread ${userPick.locked_line > 0 ? '+' : ''}${userPick.locked_line}`
      : userPick.pick_type === 'over' ? `Over ${userPick.locked_line}`
      : userPick.pick_type === 'under' ? `Under ${userPick.locked_line}` : 'ML';
    return (
      <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: 8,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          ...
          {canChange && <span onClick={onChangePick} style={{ fontSize: 10, color: "#f59e0b", cursor: "pointer", fontWeight: 600 }}>✏ Change</span>}
        </div>
        <div style={{ fontSize: 13, fontWeight: 700, color: pending ? "#60a5fa" : won ? "#22c55e" : "#ef4444" }}>
          {pending ? '⏳' : won ? '✅' : '❌'} {userPick.picked_team} — {typeLabel} {formatOdds(userPick.locked_odds)}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
          <span style={{ fontSize: 10, color: "#4a5568" }}>Wagered: {formatBucks(userPick.wager)}</span>
          {userPick.payout != null ? (
            <span style={{ fontSize: 10, fontWeight: 700, color: won ? "#22c55e" : "#ef4444" }}>
              {won ? `+${formatBucks(userPick.payout - userPick.wager)}` : `-${formatBucks(userPick.wager)}`}
            </span>
          ) : (
            <span style={{ fontSize: 10, color: "#6b7280" }}>To win: {formatBucks(calcPotentialPayout() - wager)}</span>
          )}
        </div>
...
export function NBAGameCard({ game, isExpanded, onToggle, betLog, onLogBet, user, profile }) {
  ...
  function handlePickPlaced(pick, newBankroll) {
    setUserPick(pick);
    // Update profile bankroll in parent if needed
