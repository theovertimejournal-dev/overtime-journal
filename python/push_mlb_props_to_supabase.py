...
def main():
    ...
    for sgo_player_id, prop_line_data in sgo_props.items():
        # Convert SGO ID to human name and look up MLB ID
        search_name = sgo_id_to_name(sgo_player_id)

        if search_name in mlb_id_cache:
            mlb_id, full_name, team = mlb_id_cache[search_name]
        else:
            mlb_id, full_name, team = search_mlb_player(search_name)
            mlb_id_cache[search_name] = (mlb_id, full_name, team)

        if not mlb_id:
            print(f"  ⚠ Could not resolve {sgo_player_id} → skipping", file=sys.stderr)
            continue

        # Figure out which team / game this player is in
        game_info = team_game_map.get(team, {})
        if not game_info:
            # Team abbrev mismatch — try both away and home for each game
            for g in games:
                if g["away_team"] == team or g["home_team"] == team:
                    game_info = team_game_map.get(team, {})
                    break
        if not game_info:
            print(f"  ⚠ {full_name} ({team}) not in today's lineup map — skipping")
            continue
