...
import { Avatar, AvatarMini, AvatarCreator } from '../components/common/AvatarSystem';
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          ...
          {users.map(u => (
            ...
            >
              <AvatarMini config={u.avatar_config || {}} size={32} style={{ flexShrink: 0 }} />
...
export default function ProfilePage({ currentUser, currentProfile }) {
  ...
  // FIX: Use fetched profile's user_id instead of currentProfile prop
  // currentProfile can be null during async auth loading, causing the
  // edit button to never render. The locally-fetched profile is always available.
  ...
  async function openFollowers() {
    const { data } = await supabase.from('follows')
      .select('follower_id, profiles!follows_follower_id_fkey(user_id, username, avatar_color, profile_icon, avatar_config)')
  ...
  async function openFollowing() {
    const { data } = await supabase.from('follows')
      .select('following_id, profiles!follows_following_id_fkey(user_id, username, avatar_color, profile_icon, avatar_config)')
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: FONT, padding: "24px 16px 60px", maxWidth: 640, margin: "0 auto" }}>
      ...
      {showAvatarCreator && currentUser && (
        ...
        }} onClick={(e) => { if (e.target === e.currentTarget) setShowAvatarCreator(false); }}>
          <AvatarCreator
            userId={currentUser.id}
            currentConfig={profile.avatar_config || {}}
            onSave={(newConfig) => {
              setProfile(prev => ({ ...prev, avatar_config: newConfig }));
      ...
      <div style={{
        ...
        <div style={{ position: "relative", display: "inline-block", margin: "0 auto 12px" }}>
          <Avatar config={profile.avatar_config || {}} size={90} style={{
            border: "3px solid rgba(255,255,255,0.12)",
            boxShadow: "0 0 30px rgba(196,30,58,0.2)",
            borderRadius: "50%",
          }} />
          {isOwnProfile && (
            <button onClick={() => setShowAvatarCreator(true)} style={{
              position: "absolute", bottom: -4, right: -4,
              width: 28, height: 28, borderRadius: "50%",
              background: "#c41e3a", border: "2px solid #0f0f1a",
