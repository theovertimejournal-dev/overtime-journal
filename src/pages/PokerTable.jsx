  ...
  useEffect(() => {
    ...
    async function connect() {
      try {
        ...
        if (roomCode) {
          // Join by room code — find the room first
          try {
          ...
          // No code — try to join an existing room of the same tier first
          try {
            const rooms = await client.getAvailableRooms('poker');
            const openRoom = rooms.find(r => 
              r.metadata?.tier === tier && 
              r.clients < 6 // has empty seats
            );
            if (openRoom) {
        ...
        room.onMessage('game_state', (state) => {
          ...
          // Auto sit down at first available seat (one time only)
        ...
        room.onMessage('table_info', () => {}); // suppress warning
        room.onMessage('player_joined', () => {}); // suppress warning
        room.onMessage('player_left', () => {}); // suppress warning
        room.onMessage('new_hand', () => {}); // handled via game_state
        room.onMessage('hole_cards', () => {}); // handled via game_state
        room.onMessage('player_action', () => {}); // handled via game_state
        room.onMessage('community_cards', () => {}); // handled via game_state
        room.onMessage('flop', () => {}); // handled via game_state
        room.onMessage('turn', () => {}); // handled via game_state  
        room.onMessage('river', () => {}); // handled via game_state
        room.onMessage('chips_added', () => {}); // handled via game_state
        room.onMessage('player_busted', () => {}); // handled via game_state
    ...
    return () => {
      // Only disconnect the room on unmount — don't navigate or cashout
      // Full cashout happens when user clicks LEAVE TABLE
      if (roomRef.current) {
