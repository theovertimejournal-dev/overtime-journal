...
# BDL team abbreviation → ID map
_BDL_TEAM_MAP = {}

def _get_bdl_team_map() -> dict:
    ...
    for t in data.get("data", []):
        ...
        if abbr:
            _BDL_TEAM_MAP[abbr] = t.get("id", 0)
    return _BDL_TEAM_MAP

def get_todays_games(game_date: str) -> list:
    """Get NBA games from SGO (primary) with BDL team IDs for stats lookups."""
    games = []
        ...
        if events:
            ...
            for ev in events:
                ...
                games.append({
                    ...
                    "home_team":    h,
                    "away_team":    a,
                    "home_team_id": team_map.get(h, 0),
                    "away_team_id": team_map.get(a, 0),
                    "game_time":    ev.get("startTime", "TBD"),
                    "_sgo_odds":    ev.get("odds", {}),
                })
            print(f"  SGO: {len(games)} games", file=sys.stderr)
            return games

    # BDL fallback
    print(f"  SGO unavailable — trying BDL", file=sys.stderr)
...
def get_todays_odds(game_date: str, games: list = None) -> dict:
    """Fetch odds from SGO (already embedded in games) or BDL fallback."""
    odds_by_game = {}

    # SGO odds are embedded in each game's _sgo_odds field
    if games:
        for g in games:
            sgo_odds = g.get("_sgo_odds", {})
            if not sgo_odds:
                continue
            gid = g["game_id"]
            ...
            odds_by_game[gid] = {
                ...
                "total_over_odds":  tot.get("bookOdds") or tot.get("fairOdds"),
    ...
    for row in data.get("data", []):
        ...
        cur_p = vendor_priority.index(odds_by_game[game_id].get("vendor","")) if odds_by_game[game_id].get("vendor","") in vendor_priority else 99
        new_p = vendor_priority.index(vendor) if vendor in vendor_priority else 99
        if new_p < cur_p:
            odds_by_game[game_id] = {
                "vendor":           vendor,
...
def calculate_edge(home: dict, away: dict, spread_home=0) -> dict:
    ...
    # B2B fatigue — tier and month scaled
    # Data: ELITE on road B2B still wins 67% vs avg teams
    # March B2B teams win 62% — playoff urgency overrides fatigue
    # Flat 8.0 was too harsh for elite teams in March
    from datetime import datetime as _dt_b2b
    _current_month = _dt_b2b.now().month

    def _b2b_impact(team_data, opp_data):
        """Scale B2B penalty by team tier and month."""
        wpct = team_data.get("win_pct", 0.5)
        opp_wpct = opp_data.get("win_pct", 0.5)

        # Tier multiplier — elite teams handle B2B better
        if wpct >= 0.65:      tier_mult = 0.55  # ELITE: data shows 60%→51%, not 60%→0%
        elif wpct >= 0.55:    tier_mult = 0.75  # GOOD
        elif wpct >= 0.40:    tier_mult = 1.00  # MID: full penalty
        else:                 tier_mult = 1.10  # TANK/BELOW: even worse on B2B

        # Month multiplier — March teams fight through fatigue
        if _current_month == 3:    month_mult = 0.75  # March: playoff urgency
        elif _current_month == 2:  month_mult = 1.20  # February: worst month
        elif _current_month == 4:  month_mult = 0.70  # April: seeding urgency
        else:                      month_mult = 1.00

        # Opponent quality — B2B hurts less vs bad teams (they'd win anyway)
        opp_mult = 0.85 if opp_wpct < 0.40 else 1.0

        base = 8.0
        return round(base * tier_mult * month_mult * opp_mult, 1)

    if away.get("b2b") and not home.get("b2b"):
        _impact = _b2b_impact(away, home)
        score += _impact
        _away_wpct = away.get("win_pct", 0.5)
        _tier = "ELITE" if _away_wpct >= 0.65 else "GOOD" if _away_wpct >= 0.55 else "MID"
        signals.append({
            "type": "B2B_FATIGUE",
            "detail": (
                f"{away['team']} on back-to-back"
                + (f" (March B2B — playoff urgency reduces penalty)" if _current_month == 3 else "")
                + (f" ({_tier} team handles B2B better — reduced penalty)" if _away_wpct >= 0.55 else "")
            ),
            "favors": home["team"], "strength": "STRONG", "impact": _impact,
        })
        print(f"  🔄 B2B: {away['team']} road B2B penalty={_impact} (tier={_tier}, month={_current_month})", file=sys.stderr)
    elif home.get("b2b") and not away.get("b2b"):
        _impact = _b2b_impact(home, away)
        score -= _impact
        _home_wpct = home.get("win_pct", 0.5)
        _tier = "ELITE" if _home_wpct >= 0.65 else "GOOD" if _home_wpct >= 0.55 else "MID"
        signals.append({
            "type": "B2B_FATIGUE",
            "detail": (
                f"{home['team']} on back-to-back"
                + (f" (March B2B — playoff urgency reduces penalty)" if _current_month == 3 else "")
                + (f" ({_tier} team handles B2B better — reduced penalty)" if _home_wpct >= 0.55 else "")
            ),
            "favors": away["team"], "strength": "STRONG", "impact": _impact,
        })
        print(f"  🔄 B2B: {home['team']} home B2B penalty={_impact} (tier={_tier}, month={_current_month})", file=sys.stderr)
...
def main():
    ...
    todays_odds = get_todays_odds(game_date, games=games)
    if not json_mode:
