...
def calculate_edge(home: dict, away: dict, spread_home=0) -> dict:
    ...
    # ── Star Return Warning ───────────────────────────────────────────────────
    # When a franchise star is NOT on the injury report but their team's
    # recent form (L5/L10) is way worse than expected, it likely means:
    # 1. The star was recently out and just came back
    # 2. The team's net rating/L10 was built WITHOUT the star
    # 3. The opponent's line may not reflect the star's return
    # This is the Siakam scenario — IND was 15-55 without him, returns and drops 40
    for team_data, opp_data in [(home, away), (away, home)]:
        team_abbrev = team_data["team"]
        team_injuries = team_data.get("injuries", [])
        injured_star_names = set()
        
        for inj in team_injuries:
            pname = inj.get("name", "")
            if pname in FRANCHISE_STARS:
                status = inj.get("status", "")
                if status in ("Out", "Doubtful"):
                    injured_star_names.add(pname)
        
        # Check: is there a franchise star for this team who is NOT injured?
        for star_name, (star_team, penalty) in FRANCHISE_STARS.items():
            if star_team == team_abbrev and star_name not in injured_star_names:
                # Star is available! Check if the team has been bad recently
                # (suggesting star was recently absent)
                l5_wins = team_data.get("l5_wins", 3)
                l10_wins = team_data.get("l10_wins", 5)
                ...
                try:
                    l5_pct = float(l5_wins) / 5.0
                    l10_pct = float(l10_wins) / 10.0
                except:
                    l5_pct = 0.5
                    l10_pct = 0.5
                
                # Flag if L5 is way worse than season (star was probably out recently)
                # AND the team is bad enough that the star's return matters
                if season_wpct - l5_pct >= 0.20 and penalty >= 3.0:
                    is_home = team_data["team"] == home["team"]
                    signals.append({
                        "type": "STAR_RETURN_WARNING",
                        "detail": (
                            f"⚠ {star_name} appears AVAILABLE for {team_abbrev} — "
                            f"but L5 is {int(l5_wins)}-{5-int(l5_wins)} while season is {wins}-{losses}. "
                            f"Star may be returning from absence. Team ratings and L5 data "
                            f"were built WITHOUT this player. Opponent line may not reflect the return."
                        ),
                        "favors": team_abbrev,
                        "strength": "CAUTION",
                        "impact": 0,  # informational — can't quantify return boost yet
                    })
                    print(
                        f"  🔄 STAR RETURN: {star_name} available for {team_abbrev} "
                        f"but L5 is {int(l5_wins)}-{5-int(l5_wins)} — possible return from absence",
                        file=sys.stderr
                    )
                    break  # one warning per team is enough
    # ── End Star Return Warning ───────────────────────────────────────────────
