              ...
              <div key={i}>
                {g.line_broken && (
                  <div style={{
                    ...
                    fontSize: 10, color: "#9ca3af", fontFamily: "'JetBrains Mono','SF Mono',monospace",
                    display: "flex", alignItems: "flex-start", gap: 8,
                  }}>
                    <span style={{ color: "#ef4444", fontWeight: 700, flexShrink: 0 }}>⛔ LINE BROKEN</span>
                    <span>{g.line_broken_reason || "Extreme line movement detected — late star scratch suspected. No lean issued."}</span>
                  </div>
                )}
                <NBAGameCard game={g} isExpanded={expanded[i]} onToggle={() => toggle(i, false)} betLog={betLog} onLogBet={logBet} user={user} profile={profile} />
              </div>
            ))}
          </div>
        </>
        ...
        <>
          {freeGame && (
            <div style={{ marginBottom: 20 }}>
              ...
              <NBAGameCard game={freeGame} isExpanded={true} onToggle={() => {}} betLog={[]} onLogBet={() => setShowModal(true)} user={user} profile={profile} />
            </div>
          )}
