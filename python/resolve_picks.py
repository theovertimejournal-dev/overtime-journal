...
def get_final_scores(date: str) -> dict:
    ...
    print(f"✅ Found {len(scores)} final scores for {date}")
    return scores
...
def grade_game(game: dict, scores: dict) -> dict | None:
    ...
    if spread is not None:
        try:
            ...
            # Spread covers: lean team wins by more than spread
            if lean_is_home:
                covered = margin > -spread_val  # home spread is negative when fav
            else:
                covered = (-margin) > -spread_val
            result = "W" if covered else "L"
        except (ValueError, TypeError):
            # No spread — grade straight up
...
for game in games:
    ...
    if not graded:
        matchup = game.get("matchup", "?")
        print(f"  ⚠ Skipped {matchup} — no final score or no lean")
        continue
