                ...
                <div key={`news-${item.id || i}`} style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    <span style={{
                      ...
                      {item.severity === 'breaking' ? '🚨 BREAKING' : item.type === 'march_madness' ? '🏀 MADNESS' : item.type === 'injury' ? '🏥 INJURY' : item.type === 'trade' ? '💼 TRADE' : item.type === 'line_move' ? '📈 LINE MOVE' : item.type === 'standout' ? '⭐ STANDOUT' : item.type?.includes('final') ? '🏁 FINAL' : '📰 NEWS'}
                    </span>
