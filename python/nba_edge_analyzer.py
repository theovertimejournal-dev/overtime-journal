...
def espn_get_yesterday_teams(game_date: str) -> tuple:
    ...
    Returns:
      - teams_played: set of team abbreviations that played yesterday
      - yesterday_results: dict of {abbrev: "W" or "L"} for each team
    Used to validate Balldontlie B2B detection AND patch W/L form when BDL lags.
    """
    try:
        ...
        teams_played = set()
        yesterday_results = {}  # abbrev -> "W" or "L"

        for event in data.get("events", []):
            for comp in event.get("competitions", []):
                competitors = comp.get("competitors", [])
                # Find winner — ESPN sets "winner": true on the winning competitor
                for competitor in competitors:
                    ...
                    yesterday_results[abbrev] = "W" if won else "L"
...
def check_b2b(team_id: int, game_date: str, team_abbrev: str = "", espn_yesterday: set = None, yesterday_results: dict = None) -> dict:
    ...
    yesterday_results: dict of {abbrev: "W"/"L"} from ESPN — used to prepend correct
                       result to last5 when BDL hasn't updated yet.
    """
    ...
    if not bdl_found_yesterday and espn_yesterday and team_abbrev:
        if team_abbrev.upper() in espn_yesterday:
            ...
            # Also prepend yesterday's W/L to the form string if BDL missed it
            if yesterday_results and team_abbrev.upper() in yesterday_results:
                wl = yesterday_results[team_abbrev.upper()]
                # Prepend to last5_parts so most recent game is first (cap at 5)
                last5_parts.insert(0, wl)
