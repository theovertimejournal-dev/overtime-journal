  ...
  async function loadNewspaper(date) {
    ...
    const [journalRes, slateRes, resultsRes] = await Promise.all([
      supabase.table("blog_posts")
        ...
        .limit(1)
        .then(r => r),
      supabase.table("slates")
        .select("*")
        .eq("sport", "nba")
        .eq("date", date)
        .single()
        .then(r => r).catch(() => ({ data: null })),
      supabase.table("yesterday_results")
        .select("*")
        .eq("sport", "nba")
        .order("date", { ascending: false })
        .limit(1)
      ...
      return (
        <div key={i} style={{ marginBottom: 24 }}>
          {lines.map((line, j) => {
            ...
            const isQuote = line.includes('Voice 1') || line.includes('Voice 2') || line.includes('Voice 3');
            ...
            if (isQuote) {
              ...
              const voiceColor = voiceName === 'Voice 1' ? '#60a5fa'
                : voiceName === 'Voice 2' ? '#f59e0b'
                : voiceName === 'Voice 3' ? '#22c55e' : '#94a3b8';
                ...
                <div key={j} style={{
                  ...
                  {line.split(/(Voice [123])/).map((part, k) => {
                    if (/Voice [123]/.test(part)) {
                      const color = part === 'Voice 1' ? '#60a5fa'
                        : part === 'Voice 2' ? '#f59e0b' : '#22c55e';
