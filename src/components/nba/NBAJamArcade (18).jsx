...
// ── Snapshot helpers for multiplayer state sync ──────────────────────────────
// P1 sends this compressed snapshot to P2 every few frames
function packState(s) {
  return {
    phase: s.phase,
    quarter: s.quarter, quarterTime: s.quarterTime,
    score: s.score,
    ...
    ball: {
      ...
      held: s.ball.held, holder: s.ball.holder,
      shooting: s.ball.shooting, shootProgress: s.ball.shootProgress,
      shootFrom: s.ball.shootFrom, shootTarget: s.ball.shootTarget,
      passing: s.ball.passing, passProgress: s.ball.passProgress,
      passFrom: s.ball.passFrom, passTo: s.ball.passTo, passTarget: s.ball.passTarget,
    },
    players: {
      p1: { x: s.players.p1.x, y: s.players.p1.y, team: s.players.p1.team },
      p2: { x: s.players.p2.x, y: s.players.p2.y, team: s.players.p2.team },
      c1: { x: s.players.c1.x, y: s.players.c1.y, team: s.players.c1.team },
      c2: { x: s.players.c2.x, y: s.players.c2.y, team: s.players.c2.team },
    },
    lastScore: s.lastScore, lastScoreTimer: s.lastScoreTimer,
    announcer: s.announcer, announcerTimer: s.announcerTimer,
    p1Name: s.p1Name, p2Name: s.p2Name,
  };
}

// P2 applies the snapshot to its local state (keeping local keys/touch intact)
function applySnapshot(local, snap) {
  local.phase = snap.phase;
  local.quarter = snap.quarter;
  local.quarterTime = snap.quarterTime;
  local.score = snap.score;
  local.shotClock = snap.shotClock;
  local.possession = snap.possession;
  local.activePlayer = snap.activePlayer;
  local.onFire = snap.onFire;
  local.fireActive = snap.fireActive;
  // Ball
  local.ball.x = snap.ball.x;
  local.ball.y = snap.ball.y;
  local.ball.held = snap.ball.held;
  local.ball.holder = snap.ball.holder;
  local.ball.shooting = snap.ball.shooting;
  local.ball.shootProgress = snap.ball.shootProgress;
  local.ball.shootFrom = snap.ball.shootFrom;
  local.ball.shootTarget = snap.ball.shootTarget;
  local.ball.passing = snap.ball.passing;
  local.ball.passProgress = snap.ball.passProgress;
  local.ball.passFrom = snap.ball.passFrom;
  local.ball.passTo = snap.ball.passTo;
  local.ball.passTarget = snap.ball.passTarget;
  // Players
  for (const key of ["p1","p2","c1","c2"]) {
    local.players[key].x = snap.players[key].x;
    local.players[key].y = snap.players[key].y;
    local.players[key].team = snap.players[key].team;
  }
  local.lastScore = snap.lastScore;
  local.lastScoreTimer = snap.lastScoreTimer;
  local.announcer = snap.announcer;
  local.announcerTimer = snap.announcerTimer;
  local.p1Name = snap.p1Name;
  local.p2Name = snap.p2Name;
}
...
function ChallengeFollower({ userId, profile, onChallenge }) {
  ...
  async function challengeUser(targetUser) {
    setChallenged(targetUser.username);
    setShowList(false);
    onChallenge(targetUser.username, targetUser.user_id);
  }
  ...
  return (
    <div style={{marginTop:10}}>
      <button onClick={loadFollowing} style={{
        ...
        {challenged ? `⚔️ Waiting for @${challenged}...` : "⚔️ CHALLENGE A FOLLOW"}
      </button>
      {showList && (
        <div style={{
          ...
          {following.map(u => (
            <div key={u.user_id} onClick={() => challengeUser(u)}
              style={{
...
export default function NBAJamArcade({ user, profile }) {
  ...
  // Refs that the game loop can read synchronously (useState is async)
  const mpSlotRef    = useRef(null);
  const mpStatusRef  = useRef(null);

  // P2's inputs received by P1 from the server
  const p2RemoteInput = useRef({ left:false, right:false, up:false, down:false, shoot:false, pass:false });

  // Use real username or fallback
  const myUsername = profile?.username || "Player";

  // Keep refs in sync with state
  useEffect(() => { mpSlotRef.current = mpSlot; }, [mpSlot]);
  useEffect(() => { mpStatusRef.current = mpStatus; }, [mpStatus]);

  // ── Shared room handler setup ─────────────────────────────────────────────
  const setupRoomHandlers = useCallback((room) => {
    roomRef.current = room;
    const rid = room.roomId || room.id || room.sessionId;
    setMpRoomId(rid);

    room.onMessage("joined", (d) => {
      setMpSlot(d.slot);
      mpSlotRef.current = d.slot;
      setMpStatus("waiting");
      mpStatusRef.current = "waiting";
      const confirmedId = d.roomId || rid;
      setMpRoomId(confirmedId);
      const url = new URL(window.location.href);
      url.searchParams.set("room", confirmedId);
      window.history.replaceState({}, "", url.toString());
    });
    room.onMessage("both_connected", (d) => {
      setMpStatus("ready");
      mpStatusRef.current = "ready";
      if (d.players) {
        // We need to figure out our slot from the latest value
        const mySlot = mpSlotRef.current || "p1";
        const opponent = Object.values(d.players).find(p => p.slot !== mySlot);
    ...
    room.onMessage("game_start", (d) => {
      setMpStatus("playing");
      mpStatusRef.current = "playing";
      const s = initState(); s.phase = "playing";

      // Assign names based on who is P1 vs P2
      const slot = mpSlotRef.current;
      if (slot === "p1") {
        s.p1Name = myUsername;
        // Find opponent name from the players payload
        if (d.players) {
          const opp = Object.values(d.players).find(p => p.slot === "p2");
          s.p2Name = opp?.username || "Opponent";
        } else {
          s.p2Name = "Opponent";
        }
      } else {
        // I'm P2 — P1 is the "player" team, P2 is the "cpu" team
        if (d.players) {
          const p1Player = Object.values(d.players).find(p => p.slot === "p1");
          s.p1Name = p1Player?.username || "Opponent";
        } else {
          s.p1Name = "Opponent";
        }
        s.p2Name = myUsername;
    ...
    // ── P1 receives P2's inputs ──
    room.onMessage("p2_input", (d) => {
      if (mpSlotRef.current === "p1") {
        // Apply P2's inputs so P1's game loop can use them
        p2RemoteInput.current = d;
      }
    });

    // ── P2 receives authoritative game state from P1 ──
    room.onMessage("game_state", (d) => {
      if (mpSlotRef.current === "p2") {
        applySnapshot(stateRef.current, d);
      }
    });

    // Legacy — keep for backwards compat, but not used in new flow
    room.onMessage("opponent_action", () => {});
    room.onMessage("score_update", () => {});

    room.onMessage("opponent_left", () => {
      setMpStatus(null); mpStatusRef.current = null;
      setUiPhase("menu");
    ...
    room.onMessage("game_over", () => {
      setMpStatus(null);
      mpStatusRef.current = null;
    });
    room.onMessage("rematch_start", (d) => {
      setMpStatus("playing");
      mpStatusRef.current = "playing";
      const s = initState(); s.phase = "playing";
      s.p1Name = stateRef.current.p1Name;
      s.p2Name = stateRef.current.p2Name;
      stateRef.current = s;
      setUiPhase("playing");
    });
    room.onMessage("rematch_requested", () => {
      // Could show UI — for now just auto-accept by sending rematch back
    });
  }, [myUsername]);
  ...
  const joinRoomById = useCallback(async (roomId, pin) => {
    ...
    mpStatusRef.current = "connecting";

    const timeout = setTimeout(() => {
      console.warn("Join room timed out");
      setMpStatus(null); mpStatusRef.current = null;
      setJoinError("Room not found or expired — ask your friend to create a new one");
      const u = new URL(window.location.href);
      u.searchParams.delete("room");
      window.history.replaceState({}, "", u.toString());
    }, 8000);
    ...
    try {
      ...
      setMpStatus(null); mpStatusRef.current = null;
      setJoinError("Could not join room — it may have expired");
      const u = new URL(window.location.href);
      u.searchParams.delete("room");
      window.history.replaceState({}, "", u.toString());
    }
  }, [setupRoomHandlers, myUsername]);
  ...
  const startMultiplayer = useCallback(async (targetUserId = null) => {
    setMpStatus("connecting");
    mpStatusRef.current = "connecting";
    setJoinError(null);
    try {
      ...
      const challengeRoomId = room.roomId || room.id;
      if (targetUserId && challengeRoomId) {
        try {
          await supabase.from('notifications').insert({
            user_id: targetUserId,
            type: 'arcade_challenge',
            title: `⚔️ @${myUsername} challenged you to OTJ Jam!`,
            body: 'Tap to join the game',
            data: { room_id: challengeRoomId, challenger: myUsername },
          });
        } catch (e) {
          console.warn('Challenge notification failed:', e);
        }
      }
    } catch (err) {
  ...
  }, [playerSkin, setupRoomHandlers, myUsername]);

  // ── P2: send local inputs to server every 3 frames ──
  const mpFrameCount = useRef(0);
  const syncP2Inputs = useCallback(() => {
    if (!roomRef.current || mpStatusRef.current !== "playing") return;
    if (mpSlotRef.current !== "p2") return;
    mpFrameCount.current++;
    if (mpFrameCount.current % 3 !== 0) return;
    const s = stateRef.current;
    roomRef.current.send("p2_input", {
      left:  s.keys["ArrowLeft"]  || s.touch.left  || false,
      right: s.keys["ArrowRight"] || s.touch.right || false,
      up:    s.keys["ArrowUp"]    || s.touch.up    || false,
      down:  s.keys["ArrowDown"]  || s.touch.down  || false,
      shoot: s.keys[" "]          || s.touch.shoot || false,
      pass:  s.keys["x"] || s.keys["X"] || s.touch.pass || false,
    });
  }, []);

  // ── P1: broadcast game state to P2 every 3 frames ──
  const p1BroadcastCount = useRef(0);
  const broadcastState = useCallback(() => {
    if (!roomRef.current || mpStatusRef.current !== "playing") return;
    if (mpSlotRef.current !== "p1") return;
    p1BroadcastCount.current++;
    if (p1BroadcastCount.current % 3 !== 0) return;
    roomRef.current.send("game_state", packState(stateRef.current));
  }, []);
  ...
  // ── Shoot ──────────────────────────────────────────────────────────────────
  // team: "player" or "cpu" — determines which hoop and scoring
  function handleShoot(s, team) {
    if (s.ball.shooting || s.ball.passing) return;

    if (s.possession === team && s.ball.held) {
      // Check the holder belongs to this team
      const teamPlayers = team === "player" ? ["p1","p2"] : ["c1","c2"];
      if (!teamPlayers.includes(s.ball.holder)) return;

      const ap = s.players[s.ball.holder];
      // Each team shoots at the OPPOSITE hoop
      const hoopX = team === "player" ? W - 62 : 62;
      const hoopY = H/2;
      const type = shotType(ap.x, hoopX);
      const threshold = SHOT_THRESHOLDS[type];
      const rolled = rollDice();
      const bonus = s.fireActive[team] ? FIRE_BONUS : 0;
      const willScore = (rolled + bonus) > threshold;
      s.ball.held = false;
      s.ball.shooting = true;
      s.ball.shootFrom = { x: ap.x, y: ap.y };
      s.ball.shootTarget = { x: hoopX, y: hoopY, willScore, type, shooter: team, rolled, threshold };
      s.ball.shootProgress = 0;
      if (!willScore) {
        s.announcer = `ROLL: ${rolled}+${bonus} — NEED >${threshold}`;
        s.announcerTimer = 80;
      }
    } else if (s.possession !== team) {
      // Steal attempt — the non-possessing team tries to steal
      const stealers = team === "player" ? ["p1","p2"] : ["c1","c2"];
      const holders  = team === "player" ? ["c1","c2"] : ["p1","p2"];
      // Use the active stealer (first player of team)
      const stealer = s.players[stealers[0]];
      const holder  = s.players[holders[0]];
      if (dst(stealer, holder) < 65 && rollDice() > 62) {
        s.possession = team;
        s.ball.held = true;
        s.ball.holder = stealers[0];
        s.ball.x = stealer.x; s.ball.y = stealer.y;
        s.shotClock = SHOT_CLOCK_MAX;
        s.announcer = rnd(LINES.steal); s.announcerTimer = 110;
        if (team === "player") s.activePlayer = stealers[0];
        resetPositions(s);
      }
    }
  }
  ...
  // ── Pass ───────────────────────────────────────────────────────────────────
  function handlePass(s, team) {
    if (s.ball.shooting || s.ball.passing) return;
    if (s.possession !== team) return;
    const teamPlayers = team === "player" ? ["p1","p2"] : ["c1","c2"];
    if (!s.ball.held || !teamPlayers.includes(s.ball.holder)) return;
    const from = s.ball.holder;
    const to = from === teamPlayers[0] ? teamPlayers[1] : teamPlayers[0];
    s.ball.held = false;
    s.ball.passing = true;
  ...
  useEffect(() => {
    const dn = (e) => {
      ...
      // In multiplayer as P2, don't locally process shoot/pass — just capture keys,
      // they get sent to P1 who processes them
      if (mpStatusRef.current === "playing" && mpSlotRef.current === "p2") return;

      // Determine which team I control
      const isMultiplayer = mpStatusRef.current === "playing";
      if (e.key === " ") handleShoot(s, "player");
      if (e.key === "x" || e.key === "X") handlePass(s, "player");
    };
  ...
  const touchBtn = useCallback((btn, active) => {
    ...
    // P2 in multiplayer: don't process locally, inputs go to P1
    if (mpStatusRef.current === "playing" && mpSlotRef.current === "p2") return;

    if (btn === "shoot") handleShoot(s, "player");
    if (btn === "pass")  handlePass(s, "player");
  }, [startGame]);
  ...
  useEffect(() => {
    ...
    // Track P2 shoot/pass as edge-triggered (fire once on press, not every frame)
    let p2ShootWas = false;
    let p2PassWas  = false;
    ...
    function update(dt) {
      ...
      const isMP      = mpStatusRef.current === "playing";
      const iAmP1     = mpSlotRef.current === "p1";
      const iAmP2     = mpSlotRef.current === "p2";

      // ── P2 in multiplayer: DON'T run game logic, just send inputs ──
      if (isMP && iAmP2) {
        syncP2Inputs();
        return; // State will be overwritten by game_state messages from P1
      }

      // ═══════════════════════════════════════════════════════════════
      // From here on: either SOLO mode or P1 in multiplayer
      // P1 runs the full simulation for BOTH teams
      // ═══════════════════════════════════════════════════════════════
      ...
      if (clockAcc >= 1000) {
        ...
        if (s.quarterTime === 0) {
          if (s.quarter < 4) {
            ...
            s.phase = "gameover"; setUiPhase("gameover");
            // Report final score to server
            if (isMP && iAmP1 && roomRef.current) {
              roomRef.current.send("score", { scores: { p1: s.score.player, p2: s.score.cpu } });
            }
            return;
          }
        }
      ...
      // ── "Player" team movement (P1's local keys, or solo) ──
      ...
      // Inactive "player" teammate trails
      ...
      if (s.ball.passing) {
        ...
        if (tp >= 1) {
          ...
          // If the pass target is on the "player" team, update activePlayer
          if (s.ball.passTarget === "p1" || s.ball.passTarget === "p2") {
      ...
      // ── "CPU" team logic ──
      // In multiplayer (P1 running): use P2's remote inputs for the cpu team
      // In solo: use AI
      if (isMP && iAmP1) {
        // ── MULTIPLAYER: P2 controls the "cpu" team via remote inputs ──
        const r = p2RemoteInput.current;
        const c1 = s.players.c1;

        // Movement — P2 controls c1 directly
        if (r.left)  c1.x = Math.max(22, c1.x - PLAYER_SPEED);
        if (r.right) c1.x = Math.min(W-22, c1.x + PLAYER_SPEED);
        if (r.up)    c1.y = Math.max(42, c1.y - PLAYER_SPEED);
        if (r.down)  c1.y = Math.min(H-42, c1.y + PLAYER_SPEED);

        // c2 trails c1
        s.players.c2.x += ((c1.x+55) - s.players.c2.x) * 0.06;
        s.players.c2.y += ((c1.y+35) - s.players.c2.y) * 0.06;

        // Edge-triggered shoot/pass from P2
        if (r.shoot && !p2ShootWas) handleShoot(s, "cpu");
        if (r.pass  && !p2PassWas)  handlePass(s, "cpu");
        p2ShootWas = r.shoot;
        p2PassWas  = r.pass;
      ...
      // ── P1 broadcasts state to P2 ──
      if (isMP && iAmP1) broadcastState();
    }
    ...
    function drawHUD(s) {
      ...
      // Possession indicator — in multiplayer show which team has ball
      ctx.fillStyle="#555"; ctx.font=`10px ${PIXEL_FONT}`; ctx.textAlign="left";
      if (s.possession === "player") {
        ctx.fillText(`▶ ${p1Name} ball`, 14, 25);
      } else {
        ctx.fillText(`◀ ${p2Name} ball`, 14, 25);
      }
    ...
    function draw() {
      ...
      // Determine labels based on multiplayer context
      const p1Label = s.p1Name ? s.p1Name.slice(0,6) : "OTJ 1";
      const p2Label = s.p2Name ? s.p2Name.slice(0,6) : "CPU";
      const isMP = mpStatusRef.current === "playing";

      // Player team
      drawPlayer(s.players.p2, isMP ? `${p1Label} 2` : "OTJ 2", s.fireActive.player, s.activePlayer==="p2");
      drawPlayer(s.players.p1, isMP ? `${p1Label} 1` : "OTJ 1", s.fireActive.player, s.activePlayer==="p1");
      // CPU / P2 team
      drawPlayer(s.players.c2, isMP ? `${p2Label} 2` : "CPU 2", s.fireActive.cpu, false);
      drawPlayer(s.players.c1, isMP ? `${p2Label} 1` : "CPU 1", s.fireActive.cpu, isMP);
  ...
  const handleJoystick = React.useCallback(() => {}, []);
          ...
          <div style={{
            ...
            {mpStatus === null && (
              <div style={{width:"100%",maxWidth:280,marginTop:8}}>
                ...
                {/* Challenge a follower */}
                {profile?.username && (
                  <ChallengeFollower userId={user?.id} profile={profile} onChallenge={(username, targetUserId) => {
                    startMultiplayer(targetUserId);
                  }} />
                )}
              </div>
            )}
            ...
            {mpStatus === null && joinError && (
              <div style={{marginTop:8, padding:"8px 12px", borderRadius:6, background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)"}}>
                <div style={{fontSize:10, color:"#ef4444"}}>{joinError}</div>
              </div>
            )}
            {mpStatus === "waiting" && (
