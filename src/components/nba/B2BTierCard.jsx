  ...
  return (
    <div style={{ background: "rgba(168,85,247,0.05)", border: "1px solid rgba(168,85,247,0.15)", borderRadius: 10, padding: "14px 18px", marginBottom: 14 }}>
      ...
      <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 10, lineHeight: 1.6 }}>
        Where Vegas disagrees with our model. <span style={{ color: "#ef4444", fontWeight: 600 }}>INFLATED</span> = Vegas has the spread too high, the favorite may be overpriced. <span style={{ color: "#22c55e", fontWeight: 600 }}>UNDERVALUED</span> = Vegas has the spread too low, the underdog may be getting disrespected. <span style={{ color: "#f59e0b", fontWeight: 600 }}>B2B NOT PRICED</span> = a team is playing on back-to-back nights and Vegas hasn't adjusted enough.
      </div>
      <div style={{ fontSize: 11, color: "#a855f7", fontWeight: 700, marginBottom: 10, padding: "6px 10px", background: "rgba(168,85,247,0.08)", borderRadius: 6, border: "1px solid rgba(168,85,247,0.2)" }}>
        💡 Rule of thumb: when OTJ flags a spread mismatch — take the points. Both INFLATED and UNDERVALUED point to the underdog spread.
      </div>
        ...
        {mismatches.map((m, i) => (
          <div key={i} style={{ background: "rgba(0,0,0,0.2)", borderRadius: 8, padding: "10px 14px", borderLeft: `3px solid ${m.verdictColor}` }}>
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 6, flexWrap: "wrap" }}>
              ...
              <div>
                <div style={{ fontSize: 9, color: "#4a5568" }}>OTJ FAIR SPREAD</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#60a5fa" }}>
                  {m.model_edge ? `${Math.abs(parseFloat(m.model_edge))} pts` : m.penalty}
                </div>
              </div>
