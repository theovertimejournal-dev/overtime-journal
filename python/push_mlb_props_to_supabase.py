...
MIN_GAMES_PLAYED  = 5     # Min games before we'll score a batter's HR prop
...
def get_sgo_hr_props():
    ...
    for event in events:
        ...
        for odd_id, odd_data in odds_dict.items():
            # odd_id format: batting_homeRuns-PLAYER_ID-game-ou-over
            if "batting_homeRuns" not in odd_id or "-game-ou-over" not in odd_id:
                continue
            parts = odd_id.split("-")
            if len(parts) < 2:
                continue
            player_id = parts[1]  # e.g. SHOHEI_OHTANI_1_MLB

            over_under_val = odd_data.get("overUnderValue")
            ...
            for book in VENDOR_PRIORITY:
                ...
                if entry.get("available") and entry.get("odds") is not None:
                    over_odds = int(entry["odds"])
                    vendor = book
                    break

            # Get opposing (under) odds
            ...
            for book in VENDOR_PRIORITY:
                ...
                if entry.get("available") and entry.get("odds") is not None:
                    under_odds = int(entry["odds"])
                    break

            # Figure out which team this player is on
            player_team = None
            player_name_parts = player_id.replace("_MLB", "").split("_")
            # SGO player IDs end in team abbrev sometimes — or we match by event teams
            # For now, store both teams and resolve later via MLB Stats API
            props[player_id] = {
                "line":       float(over_under_val),
                "over_odds":  over_odds or -115,
                "under_odds": under_odds or -105,
                "vendor":     vendor or "consensus",
                "away_team":  away_abbr,
                "home_team":  home_abbr,
                "game_pk":    None,  # filled in when we match to a game
            }
...
def main():
    ...
    for sgo_player_id, prop_line_data in sgo_props.items():
        # TODO: Map SGO player ID to MLB Stats API player ID
        # SGO format: SHOHEI_OHTANI_1_MLB → search MLB Stats API by name
        # For now we need a mapping table or search by name
        # This is the primary blocker — see open questions in PRD

        # Placeholder: skip until SGO → MLB ID mapping is implemented
        # This will be filled in once we see actual SGO HR prop response format
        continue

    # ── FALLBACK: Score top HR hitters even without SGO lines ──────────────────
    # Used during Opening Week when SGO may not have lines yet
    print("  ⏳ Fallback: scoring top HR hitters from today's lineup...")
    for g in games:
        for side in ["away", "home"]:
            ...
            pitcher_stats = get_pitcher_stats(starter_id)

            # Get roster for this team, find top HR hitters
            roster_data = mlb_get(f"/teams/{g[f'{side}_id']}/roster", {
                "rosterType": "active", "season": datetime.now().year
            })
            for player in roster_data.get("roster", [])[:13]:  # Top 13 (batting roster)
                pos = player.get("position", {}).get("abbreviation", "")
                if pos in ("P", "RP", "SP"):
                    continue  # Skip pitchers

                mlb_id   = player.get("person", {}).get("id")
                p_name   = player.get("person", {}).get("fullName", "Unknown")
                if not mlb_id:
                    continue
                ...
                if batter_stats.get("season_games", 0) < MIN_GAMES_PLAYED:
                    continue
                # Skip very low HR rate players
                if batter_stats.get("season_hr_rate", 0) < 0.02:
                    continue
                ...
                # Check if we have SGO odds for this player
                # (name matching — will improve with proper ID mapping)
                over_odds  = -115
                under_odds = -105
                line       = 0.5
                vendor     = "none"
