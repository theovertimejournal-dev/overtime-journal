...
def get_injury_reaction(voice, name, team, is_fresh, desc):
    ...
    if is_star and is_fresh:
        ...
        reactions = {
            "yumi": f"Here's the thing — {name} being out tonight changes everything for {team}. This isn't minor.",
            "johnnybot": f"{name} out? Watch the line move in the next 20 minutes. Vegas hasn't caught up yet.",
            "krash": f"No {name} tonight. That hurts to type. {team} is a completely different team without him on the floor.",
        }
    elif is_star:
        # Known star, already priced in
        reactions = {
            "yumi": f"{name} still out for {team}. At this point the line reflects it, but the team misses what he brings.",
            "johnnybot": f"Everyone knows {name} is out. The question is whether {team} has figured out how to play without him yet.",
            "krash": f"Another game without {name}. {team} keeps fighting but you can feel the difference.",
        }
    elif is_fresh:
        # Non-star fresh scratch
        reactions = {
            "yumi": f"{name} scratched late for {team}. Rotation just got shorter.",
            "johnnybot": f"{name} out tonight — how much does that actually matter for {team}? Depends on the matchup.",
            "krash": f"Hate to see anyone go down. {name} out for {team} tonight.",
        }
    else:
        # Routine bench/role player injury — quick one-liner
        reactions = {
            "yumi": f"{team} down another body. Depth matters in March.",
            "johnnybot": f"Another name on the {team} injury list. It's getting crowded.",
            "krash": f"{team} thin tonight. Next man up.",
        }
    ...
    try:
        ...
        for article in news_data.get("articles", [])[:5]:
            ...
            injury_keywords = ["injury", "injured", "out for", "ruled out", "day-to-day",
                               "trade", "traded", "signs with", "waived", "released", "suspended",
                               "IL", "disabled list", "concussion", "ACL", "MCL", "surgery",
                               "free agent", "contract", "extension"]
            # Exclude junk headlines
            junk_keywords = ["fantasy", "rankings", "rest-of-season", "mock draft", "odds to win",
                             "betting", "picks to make", "card sells", "logoman", "rookie card",
                             "podcast", "mailbag", "power rankings", "tier list", "redraft"]
            combined = (headline_text + desc).lower()
            if not any(k.lower() in combined for k in injury_keywords):
                continue
            if any(k.lower() in combined for k in junk_keywords):
                continue
