    ...
    for r in all_results:
        ...
        if gap >= 2.0 and is_significant:
            ...
            if spread_val < 0 and model_implied > 0:
                ...
                verdict = (
                    f"Vegas: {home['team']} -{abs(spread_val)}. "
                    f"Net ratings project a {abs(model_implied):.1f}-pt game (signal score: {round(edge_score,1)}). "
                    f"The {abs(model_implied):.1f}-pt projection is the fair spread — "
                    f"the signal score ({round(edge_score,1)}) measures how many model signals fired, not a point margin. "
                    f"Spread appears inflated — fade value on {away['team']}."
                )
                ...
                verdict = (
                    f"Vegas: {home['team']} +{spread_val}. "
                    f"Net ratings project a {abs(model_implied):.1f}-pt game (signal score: {round(edge_score,1)}). "
                    f"The {abs(model_implied):.1f}-pt projection is the fair spread — "
                    f"the signal score ({round(edge_score,1)}) measures how many model signals fired, not a point margin. "
                    f"Line may be soft — value on {home['team']}."
                )
            elif has_b2b:
            ...
            spread_mismatches.append({
                ...
                # model_edge = net rating implied spread (net_diff * 0.45)
                # This is what the gut check also uses — how many pts the net ratings say
                "model_edge": f"{model_implied:+.1f} pts",
                # edge_score = raw signal accumulator (not a point margin)
                # Higher = more signals firing, NOT a point total
...
def main():
    ...
    for game in games:
        ...
        try:
            ...
            if abs(ml_h_val) >= 3000 or abs(ml_a_val) >= 3000:
                line_broken = True
