...
def calculate_edge(home: dict, away: dict, spread_home=0) -> dict:
    ...
    # ── Positional Mismatch Signal ────────────────────────────────────────────
    # Fires when a franchise star is active AND facing a team whose starter at
    # that position is out, leaving a weak backup as the primary defender.
    # Logic: star active + opposing starter out (any tenure) + backup bench net
    # below threshold = structural mismatch the line may not fully capture.
    # Weight: LOW-MODERATE (2.0-3.5) — supporting signal, never lead signal.
    # Only fires pre-game (not on already-priced long-term absences alone).

    STAR_POSITIONS = {
        # star name → (team, position_type, impact)
        "Giannis Antetokounmpo": ("MIL", "big",   3.0),
        "Joel Embiid":           ("PHI", "big",   3.0),
        "Nikola Jokic":          ("DEN", "big",   3.5),
        "Victor Wembanyama":     ("SAS", "big",   3.5),
        "Anthony Davis":         ("LAL", "big",   3.0),
        "Karl-Anthony Towns":    ("NYK", "big",   2.5),
        "Bam Adebayo":           ("MIA", "big",   2.5),
        "Jayson Tatum":          ("BOS", "wing",  2.5),
        "Luka Doncic":           ("DAL", "wing",  3.0),
        "LeBron James":          ("LAL", "wing",  2.5),
        "Kevin Durant":          ("PHO", "wing",  2.5),
        "Kawhi Leonard":         ("LAC", "wing",  2.5),
        "Ja Morant":             ("MEM", "guard", 2.5),
        "Stephen Curry":         ("GSW", "guard", 2.5),
        "Damian Lillard":        ("MIL", "guard", 2.5),
    }
    ...
    for team_data, opp_data in [(home, away), (away, home)]:
        ...
        team_injuries = team_data.get("injuries", [])
        opp_injuries  = opp_data.get("injuries", [])
        opp_bench_net = opp_data.get("bench_net", 0) or 0

        for star_name, (star_team, pos_type, base_impact) in STAR_POSITIONS.items():
            if star_team != team_abbrev:
                continue

            # Star must be ACTIVE (not in injury list)
            star_out = any(
                p.get("name") == star_name and p.get("status") in ("Out", "Doubtful")
                for p in team_injuries
            )
            if star_out:
                continue  # star isn't playing — no mismatch to exploit

            # Check if opposing team's positional starter is out
            # Proxy: look for Out/Doubtful players on opp who play that position
            # We use description keywords as position proxy since BDL doesn't give pos
            BIG_KEYWORDS   = ["center", "forward", "pf", "sf", "power", "stretch"]
            GUARD_KEYWORDS = ["guard", "pg", "sg", "point", "shooting"]
            WING_KEYWORDS  = ["forward", "wing", "sf", "sg", "small"]

            pos_keywords = {
                "big":   BIG_KEYWORDS,
                "guard": GUARD_KEYWORDS,
                "wing":  WING_KEYWORDS,
            }.get(pos_type, [])

            # Find opp starters who are out at this position
            opp_starters_out = [
                p for p in opp_injuries
                if p.get("status") in ("Out", "Doubtful")
                and any(kw in (p.get("description", "") + p.get("name", "")).lower()
                        for kw in pos_keywords)
            ]

            # Also fire if opp bench net is very weak (below -3) regardless of injury
            # — implies their backup unit is thin even without specific injury flag
            weak_backup = opp_bench_net < -3.0

            if not opp_starters_out and not weak_backup:
                continue

            # Calculate impact — boost if both conditions true
            impact = base_impact
            if opp_starters_out and weak_backup:
                impact = round(min(impact + 1.0, 4.5), 1)
            elif weak_backup and not opp_starters_out:
                impact = round(impact * 0.6, 1)  # weaker signal without confirmed injury

            is_home_star = team_abbrev == home["team"]
            if is_home_star:
                score += impact
            else:
                score -= impact

            detail_parts = [f"{star_name} ({team_abbrev}) faces"]
            if opp_starters_out:
                out_names = ", ".join(p["name"] for p in opp_starters_out[:2])
                detail_parts.append(f"depleted {pos_type} ({out_names} out)")
            if weak_backup:
                detail_parts.append(f"{opp_abbrev} bench net {opp_bench_net:+.1f} (weak backup unit)")

            signals.append({
                "type":     "POSITIONAL_MISMATCH",
                "detail":   " — ".join(detail_parts),
                "favors":   team_abbrev,
                "strength": "MODERATE" if impact >= 3.0 else "MILD",
                "impact":   impact,
            })
            print(
                f"  📐 POSITIONAL MISMATCH: {star_name} vs weak {pos_type} "
                f"({opp_abbrev} bench net {opp_bench_net:+.1f})",
                file=__import__("sys").stderr,
            )
            break  # one signal per team per game
    # ── End Positional Mismatch Signal ────────────────────────────────────────
...
def build_parlay(all_results: list, spread_mismatches: list) -> dict | None:
    ...
    def build_leg(result, label, mismatch):
        ...
        # ── Gut check override ────────────────────────────────────────────────
        # If gut check fires with a gap >= 4pts, the VALUE is on the DOG spread,
        # not the lean team's spread. Parlay should flip to the dog side.
        # This prevents the contradiction: gut check says "take the dog +13.5"
        # while the parlay says "take the favorite -13.5".
        otj_spread_rule = edge.get("otj_spread_rule")
        gut_check_flip = (
            otj_spread_rule and
            otj_spread_rule.get("gap", 0) >= 4.0 and
            otj_spread_rule.get("dog_team")
        )
        if gut_check_flip:
            dog_team = otj_spread_rule["dog_team"]
            dog_is_home = dog_team == result["home"]["team"]
            pick_team = dog_team
            spread = result.get("spread_home") if dog_is_home else result.get("spread_away")
            spread_odds = result.get("spread_home_odds") if dog_is_home else result.get("spread_away_odds")
            ml = result.get("ml_home") if dog_is_home else result.get("ml_away")
            # Use padded spread if available — that's where the real value is
            padded = otj_spread_rule.get("padded_spread")
            pick_line = f"{dog_team} {padded}" if padded else f"{dog_team} {float(spread):+.1f}" if spread else dog_team
            pick_type = "spread"
            gut_check_note = f"Gut check: model sees {otj_spread_rule.get('fair_spread')}pt game vs Vegas {otj_spread_rule.get('vegas_spread')} — dog has value"
        else:
            pick_team = lean_team
        ...
        return {
            ...
            "top_signal": gut_check_note or (signals[0]["detail"] if signals else "Model edge"),
            "mismatch_gap": mismatch.get("gap") if mismatch else None,
            "gut_check_flip": gut_check_flip,
        }
