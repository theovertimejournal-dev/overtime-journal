...
def build_parlay(all_results: list, spread_mismatches: list) -> dict | None:
    ...
    PARLAY ELIGIBILITY RULES:
      1. GUT CHECK VETO — if posted spread vs model implied spread differs by 5+ pts,
         that game is ineligible as a spread parlay leg.
      2. ML VALUE FILTER — if lean team ML is worse than -400, switch to spread.
         If ML is between -400 and -150, prefer spread for better value.
      All diagnostic prints go to stderr so JSON stdout stays clean.
    """
    ...
    def is_gut_check_vetoed(result) -> bool:
        ...
        if gap >= 5.0:
            print(f"  Parlay veto -- {matchup} spread gap {gap:.1f} pts too large (gut check)", file=sys.stderr, flush=True)
            return True
        return False
        ...
        if ml_val is not None:
            if ml_val <= -400:
                if score >= 14 and spread is None:
                    ...
                    print(f"  Parlay swap -- {lean_team} ML {ml_val:+d} to spread (ML too heavy)", file=sys.stderr, flush=True)
                    return "spread", spread_line, ml_val
                else:
                    print(f"  Parlay skip -- {lean_team} ML {ml_val:+d} too heavy, no spread available", file=sys.stderr, flush=True)
