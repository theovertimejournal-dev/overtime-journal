/**
 * PokerLobby.jsx — OTJ Poker Lobby (v2)
 * 
 * Fixes:
 *  - Refreshes bankroll on mount (catches post-poker balance changes)
 *  - Cleaner error handling on buy-in deduction
 *  - Validates bankroll server-side before navigating
 */
...
export default function PokerLobby({ user, profile }) {
  ...
  const [liveBankroll, setLiveBankroll] = useState(profile?.bankroll || 0);
  const navigate = useNavigate();

  const tier = TIERS[selectedTier];
  const canAfford = liveBankroll >= tier.minBuy;

  // Refresh bankroll on mount — catches post-poker changes from server
  useEffect(() => {
    if (user?.id) {
      supabase.from('profiles').select('bankroll').eq('user_id', user.id).single()
        .then(({ data }) => { if (data) setLiveBankroll(data.bankroll || 0); });
    }
  }, [user?.id]);

  // Also update from prop changes
  useEffect(() => {
    if (profile?.bankroll != null) setLiveBankroll(profile.bankroll);
  }, [profile?.bankroll]);
  ...
  async function handleJoinTable() {
    ...
    try {
      // Re-fetch fresh bankroll to avoid stale data
      ...
      const currentBankroll = freshProfile?.bankroll || 0;
      if (currentBankroll < buyIn) {
        setError(`Not enough OTJ Bucks! You have ${formatBucks(currentBankroll)}`);
        setLiveBankroll(currentBankroll);
        setJoining(false);
        return;
      }
      ...
      const { error: updateErr } = await supabase
        .from('profiles')
        .update({ bankroll: currentBankroll - buyIn })
        .eq('user_id', user.id);

      if (updateErr) throw updateErr;

      // Log to ledger (non-critical)
      try {
        await supabase.from('bucks_ledger').insert({
          user_id: user.id,
          type: 'poker_buyin',
          amount: -buyIn,
          balance_after: currentBankroll - buyIn,
          note: `Poker buy-in (${tier.label} table)`,
        });
      } catch {}

      setLiveBankroll(currentBankroll - buyIn);
    ...
    <div style={{ minHeight: '100vh', fontFamily: FONT, padding: '24px 16px 60px', maxWidth: 640, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: '#22c55e', marginTop: 2 }}>{formatBucks(liveBankroll)}</div>
        </div>
        {profile && <AvatarMini config={profile.avatar_config} size={40} />}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
        {Object.entries(TIERS).map(([key, t]) => {
          ...
          const affordable = liveBankroll >= t.minBuy;
          return (
            <div
              key={key}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <input
          type="range"
          min={tier.minBuy}
          max={Math.min(tier.maxBuy, liveBankroll)}
          step={tier.blinds[1]}
          value={buyIn}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: '#374151', marginTop: 4 }}>
          ...
          <span>Max: {formatBucks(Math.min(tier.maxBuy, liveBankroll))}</span>
        </div>
      </div>
