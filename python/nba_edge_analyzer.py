...
def build_parlay(all_results: list, spread_mismatches: list) -> dict | None:
    ...
    PARLAY ELIGIBILITY RULES (prevents bad legs like MIA -15.5 or DET -1050):
      1. GUT CHECK VETO — if a game's posted spread differs from the model's
         implied spread by 5+ pts, that game is INELIGIBLE as a spread parlay leg.
         The gut check means "Vegas is pricing something we don't see" — don't fight it.
      2. ML VALUE FILTER — if the lean team's ML is worse than -400, we only include
         it as an ML leg if model confidence justifies it (score >= 14). Otherwise
         we switch to the spread leg for that game, which expresses the same edge
         with way better value. A -1050 ML on a 7-pt model game is never a good bet.
      3. SPREAD PREFERENCE on heavy favorites — if ML is between -400 and -150,
         prefer the spread. If ML is better than +100 or between -150 and 0, ML is fine.
    """

    # ── Pre-compute gut check veto set ────────────────────────────────────────
    # Build a lookup of matchup -> gap in pts (model implied vs Vegas spread)
    # Any game where gap >= 5 gets vetoed from spread legs in the parlay.
    mismatch_gap_lookup = {}
    for m in spread_mismatches:
        try:
            gap_val = float(str(m.get("gap", "0")).replace(" pts", ""))
            mismatch_gap_lookup[m["matchup"]] = gap_val
        except (ValueError, TypeError):
            mismatch_gap_lookup[m["matchup"]] = 0.0

    def is_gut_check_vetoed(result) -> bool:
        """Returns True if the spread gap is too large to trust as a parlay leg."""
        matchup = result.get("matchup", "")
        gap = mismatch_gap_lookup.get(matchup, 0.0)
        if gap >= 5.0:
            print(f"  ⚠️  Parlay veto — {matchup} spread gap {gap:.1f} pts too large (gut check)", flush=True)
            return True
        return False

    def get_best_pick_type(result) -> tuple:
        """
        Decides whether to use ML or spread for this leg.
        Returns (pick_type, pick_line, ml_val_or_none)
        Rules:
          - ML worse than -400 AND score < 14 → use spread instead
          - ML between -400 and -150 → prefer spread (better value)
          - ML >= -150 or positive → ML is fine
          - If spread is gut-check vetoed, skip ML heavy favorites too
        """
        edge = result["edge"]
        ...
        ml_raw = result.get("ml_home") if is_home else result.get("ml_away")
        score = abs(edge.get("score", 0))

        ml_val = None
        try:
            ml_val = int(ml_raw) if ml_raw is not None else None
        except (ValueError, TypeError):
            ml_val = None

        spread_line = f"{lean_team} {float(spread):+.1f}" if spread is not None else None
        ml_line = f"{lean_team} {ml_val:+d}" if ml_val is not None else None

        if ml_val is not None:
            if ml_val <= -400:
                # Very heavy favorite — only use ML if model is very confident
                if score >= 14 and spread is None:
                    return "ml", ml_line, ml_val
                elif spread_line:
                    print(f"  💡 Parlay swap — {lean_team} ML {ml_val:+d} → spread (ML too heavy, use spread value)", flush=True)
                    return "spread", spread_line, ml_val
                else:
                    # No spread available — skip this leg entirely (too risky)
                    print(f"  ⚠️  Parlay skip — {lean_team} ML {ml_val:+d} too heavy, no spread available", flush=True)
                    return None, None, ml_val
            elif ml_val <= -150:
                # Moderate favorite — prefer spread for value
                if spread_line:
                    return "spread", spread_line, ml_val
                else:
                    return "ml", ml_line, ml_val
            else:
                # ML is +EV or near even — take ML
                return "ml", ml_line, ml_val
        elif spread_line:
            return "spread", spread_line, None
        else:
            return None, lean_team, None

    # ── Sort candidates ────────────────────────────────────────────────────────
    sharp_games = sorted(
        [r for r in all_results if r["edge"]["confidence"] == "SHARP" and not is_gut_check_vetoed(r)],
        key=lambda r: abs(r["edge"]["score"]), reverse=True
    )
    lean_games = sorted(
        [r for r in all_results if r["edge"]["confidence"] == "LEAN" and not is_gut_check_vetoed(r)],
        key=lambda r: abs(r["edge"]["score"]), reverse=True
    )
    ...
    # Leg 3: biggest mismatch gap on a different game (already have gap data)
    # For leg 3 (VALUE leg) the mismatch IS the point — we want the gap.
    # But still apply gut-check veto: if gap is >= 5 AND direction is INFLATED,
    # we flip to the dog spread instead of the favored spread.
    ...
    # If no mismatch leg, fall back to next best scored non-vetoed game
    if not leg3_result:
        remaining = sorted(
            [r for r in all_results if r["matchup"] not in used and not is_gut_check_vetoed(r)],
            key=lambda r: abs(r["edge"]["score"]), reverse=True
        )
    ...
    def build_leg(result, label, mismatch):
        ...
        # For value legs (mismatch), always use spread — that's the whole point
        if mismatch:
            is_home = lean_team == result["home"]["team"]
            spread = result.get("spread_home") if is_home else result.get("spread_away")
            if spread is not None:
                ...
                pick_type = "ml"
                ml_raw = result.get("ml_home") if is_home else result.get("ml_away")
                pick_line = f"{lean_team} {int(ml_raw):+d}" if ml_raw else lean_team
        else:
            # Use the smart pick type selector for non-value legs
            pick_type, pick_line, _ = get_best_pick_type(result)
            if pick_type is None:
                return None  # vetoed by ML filter
