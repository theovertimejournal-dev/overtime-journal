        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 40, paddingTop: 36 }}>
          ...
          <div>
            ...
            {(() => {
              ...
              else if (feedFilter === 'scores') filtered = liveFeed.filter(n => n.type === 'game_final' || n.type === 'march_madness');
              else if (feedFilter === 'standout') filtered = liveFeed.filter(n => n.type === 'standout');
              else if (feedFilter === 'journal') filtered = []; // journal only shows blog posts above
                ...
                <div key={`news-${item.id || i}`} style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    <span style={{
                      ...
                      {item.severity === 'breaking' ? '🚨 BREAKING' : item.type === 'march_madness' ? '🏀 MADNESS' : item.type === 'injury' ? '🏥 INJURY' : item.type === 'line_move' ? '📈 LINE MOVE' : item.type === 'standout' ? '⭐ STANDOUT' : '🏁 FINAL'}
                    </span>
                    ...
                    <div style={{ fontSize: 11, color: '#6b7280', lineHeight: 1.6, fontStyle: 'italic' }}>
                      {item.character && (
                        <span style={{
                          ...
                          color: item.character === 'voice_1' ? '#60a5fa' : item.character === 'voice_2' ? '#f59e0b' : '#22c55e',
                          marginRight: 6,
                        }}>
                          {item.character === 'voice_1' ? 'V1' : item.character === 'voice_2' ? 'V2' : 'V3'}
                        </span>
                      )}
                      {item.body}
                    </div>
