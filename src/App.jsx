...
export default function App() {
  ...
  useEffect(() => {
    ...
    // ─── Core auth helper — fetch profile then unblock the UI ────────────────
    // Called from multiple event paths (SIGNED_IN, TOKEN_REFRESHED, INITIAL_SESSION).
    // Single source of truth so Chrome hard-refresh and normal login share same logic.
    async function resolveUser(u) {
      ...
      if (fetchingProfile) {
        // Already in-flight — just unblock, the other call will set everything
        setAuthChecked(true);
        setSessionValidated(true);
        return;
      }
      ...
      try {
        ...
        // PGRST116 = no rows = new user, show username modal
        // Any other error = network/auth issue, skip modal silently
    ...
    // ─── On mount: read current session immediately (no refresh call) ────────
    // Chrome hard refresh: getSession() returns the cached session synchronously.
    // We DON'T call refreshSession() here — that was causing Chrome to swallow
    // the subsequent onAuthStateChange event, leaving the app stuck on LOADING.
    // Supabase auto-refreshes tokens internally when they're close to expiry.
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        // No session at all — logged out, unblock immediately
        setAuthChecked(true);
        setSessionValidated(true);
        setProfileLoading(false);
        setShowWelcome(true);
      }
      // If session exists, onAuthStateChange will fire INITIAL_SESSION shortly
      // and resolveUser() will handle everything from there.
    });
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      // INITIAL_SESSION fires on every page load with the current session state.
      // TOKEN_REFRESHED fires when Supabase silently rotates the token.
      // Both just need to resolve the user — same path.
      if (event === 'INITIAL_SESSION' || event === 'TOKEN_REFRESHED') {
        if (!u) {
          // Logged out state confirmed
          setShowWelcome(true);
      ...
      // Any other event (PASSWORD_RECOVERY etc) — just resolve
      await resolveUser(u);
    });
