...
def fetch_record_range(start: str, end: str) -> tuple:
    """Sum W/L from all slates in a date range — excludes grade_date to avoid double counting."""
    try:
...
print(f"\n{'=' * 60}")
print(f"  ✅ Model resolve complete for {grade_date}")
print(f"  Yesterday: {yesterday_record} | Cumulative: {new_cumulative} | Streak: {streak}")
print(f"{'=' * 60}\n")


# ============================================================================
# STEP 8: RESOLVE USER BETS (game_picks table)
# ============================================================================
# Grades all user-placed bets for the grade_date against final scores.
# Updates: game_picks.result, game_picks.net, profiles.bankroll
# This is what powers the leaderboard, profiles, and P/L tracking.

print(f"⏳ Resolving user bets for {grade_date}...")

try:
    # Fetch all ungraded picks for this date
    picks_resp = supabase.table("game_picks") \
        .select("*") \
        .eq("slate_date", grade_date) \
        .is_("result", "null") \
        .execute()

    pending_picks = picks_resp.data or []
    print(f"  Found {len(pending_picks)} ungraded user bets for {grade_date}")

    if not pending_picks:
        print(f"  ✅ No user bets to grade")
    else:
        bet_wins = 0
        bet_losses = 0
        bet_errors = 0
        ...
        for pick in pending_picks:
            ...
            locked_odds = pick.get("locked_odds", 0)
            locked_line = pick.get("locked_line")
            wager = float(pick.get("wager", 0) or 0)

            # Find the final score for this game
            if matchup not in scores:
                print(f"  ⏳ PENDING bet #{pick_id} — {matchup} score not found yet")
                bet_errors += 1
                continue
            ...
            # Figure out which team is home/away from matchup string "AWAY @ HOME"
            parts = matchup.split(" @ ")
            if len(parts) != 2:
                print(f"  ⚠ Bad matchup format: {matchup}")
                bet_errors += 1
                continue
            away_team, home_team = parts[0].strip(), parts[1].strip()

            picked_is_home = picked_team == home_team

            # ── Grade based on bet type ──
            bet_won = False

            if pick_type == "moneyline":
                # Did the picked team win straight up?
                if picked_is_home:
                ...
                # locked_line is from the picker's perspective (e.g., +8.5 or -3.5)
                try:
                    line = float(locked_line or 0)
                    if picked_is_home:
                        # Home team: margin + line > 0 means cover
                        bet_won = (margin + line) > 0
                    else:
                        # Away team: (-margin) + line > 0 means cover
                        bet_won = (-margin + line) > 0
                except (ValueError, TypeError):
                    print(f"  ⚠ Bad spread line for bet #{pick_id}: {locked_line}")
                    bet_errors += 1
                    continue
                ...
                print(f"  ⚠ Unknown pick_type: {pick_type}")
                bet_errors += 1
                continue
            ...
            if bet_won:
                odds = int(locked_odds or 0)
                if odds > 0:
                    profit = round(wager * odds / 100, 2)
                elif odds < 0:
                    profit = round(wager * 100 / abs(odds), 2)
                else:
                    profit = wager  # even money
                net = profit
            else:
                net = -wager
            ...
            try:
                ...
                print(f"  ⚠ Failed to update pick #{pick_id}: {e}")
                bet_errors += 1
                continue
            ...
            try:
                ...
                prof_resp = supabase.table("profiles") \
                    .select("bankroll") \
                    .eq("user_id", user_id) \
                    .single() \
                    .execute()
                current_bankroll = float(prof_resp.data.get("bankroll", 0) or 0)

                if bet_won:
                    # Return wager + profit (wager was already deducted on placement)
                    new_bankroll = round(current_bankroll + wager + profit, 2)
                else:
                    # Wager was already deducted on placement, nothing to return
                ...
                try:
                    supabase.table("bucks_ledger").insert({
                        ...
                        "type": "bet_result",
                        "note": f"{'W' if bet_won else 'L'} — {picked_team} {pick_type} ({matchup})",
                    }).execute()
                except Exception:
                    pass  # ledger is non-critical

            except Exception as e:
                print(f"  ⚠ Failed to update bankroll for user {user_id}: {e}")

            if bet_won:
                bet_wins += 1
                print(f"  ✅ BET W — {picked_team} {pick_type} | {matchup} | +${net:.0f}")
            else:
                bet_losses += 1
                print(f"  ❌ BET L — {picked_team} {pick_type} | {matchup} | -${wager:.0f}")

        print(f"\n  📊 User bets: {bet_wins}W {bet_losses}L ({bet_errors} errors/pending)")

except Exception as e:
    print(f"  ❌ User bet resolution failed: {e}")

print(f"\n{'=' * 60}")
print(f"  ✅ Full resolve complete for {grade_date}")
print(f"  Model: {yesterday_record} | Cumulative: {new_cumulative} | Streak: {streak}")
print(f"{'=' * 60}\n")
