...
def pull_data(date: str) -> dict:
    ...
    # Today's slate preview — pull FULL data including games array
    ...
    try:
        ...
        today = subprocess.check_output(
            ["date", "+%Y-%m-%d"], env={**os.environ, "TZ": "America/New_York"}
        ).decode().strip()

    print(f"  Today's slate date: {today}", file=sys.stderr)
    try:
        resp = supabase.table("slates") \
            .select("*") \
            .eq("date", today).eq("sport", "nba") \
            .execute()
        data["today_slate"] = resp.data[0] if resp.data else None
        if data["today_slate"]:
            games = data["today_slate"].get("games", [])
            print(f"  ✅ Found today's slate: {len(games)} games", file=sys.stderr)
        else:
            print(f"  ⚠ No slate found for {today}", file=sys.stderr)
    except Exception as e:
        print(f"  ⚠ Slate fetch error: {e}", file=sys.stderr)
...
def build_master_prompt(data: dict, date: str) -> str:
    ...
    # ── Build tonight's preview text ──
    tonight_text = ""
    tonight_games = (today_slate or {}).get("games", [])
    if tonight_games:
        tonight_text += f"{len(tonight_games)} games tonight.\n\n"
        for g in tonight_games:
            edge = g.get("edge", {})
            matchup = g.get("matchup", "???")
            lean = edge.get("lean", "")
            conf = edge.get("confidence", "")
            score = edge.get("edge_score", 0)
            
            # B2B flags
            away_b2b = g.get("away", {}).get("b2b", False)
            home_b2b = g.get("home", {}).get("b2b", False)
            away_team = g.get("away", {}).get("team", "")
            home_team = g.get("home", {}).get("team", "")
            
            # Injuries
            away_inj = [i.get("name", "") for i in g.get("away", {}).get("injuries", []) if i.get("tenure") == "fresh"]
            home_inj = [i.get("name", "") for i in g.get("home", {}).get("injuries", []) if i.get("tenure") == "fresh"]
            
            line = f"  {matchup} — Lean: {lean} ({conf}, score {score})"
            if away_b2b:
                line += f" | {away_team} on B2B 🔴"
            if home_b2b:
                line += f" | {home_team} on B2B 🔴"
            if away_inj:
                line += f" | {away_team} fresh OUT: {', '.join(away_inj)}"
            if home_inj:
                line += f" | {home_team} fresh OUT: {', '.join(home_inj)}"
            tonight_text += line + "\n"
    else:
        tonight_text = "Tonight's slate hasn't dropped yet."
...
═══ TONIGHT'S GAMES (characters should react to these — call out what excites them, what worries them, what to watch) ═══
{tonight_text}
...
8. THE LAST SECTION MUST BE ABOUT TONIGHT'S GAMES — the characters preview what's coming:
   - Call out specific matchups they're excited about
   - React to B2B situations ("Luka on a back-to-back? Krash is worried.")
   - Flag fresh injuries that change things
   - Each character can have a different game they're watching
...
12. ONLY mention a player doing something in a game if they appear in the STANDOUT PERFORMERS data above with real stats. If a player is NOT listed in the box score data, DO NOT mention them playing, scoring, assisting, blocking, or doing ANYTHING in that game.
13. If a star player is NOT in the standout performers list, they either did not play or had a quiet game. Do NOT assume they played. Do NOT invent stat lines or clutch moments for them.
