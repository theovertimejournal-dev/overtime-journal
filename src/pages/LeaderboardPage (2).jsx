...
export default function LeaderboardPage({ currentUser }) {
  ...
  async function loadLeaderboard() {
    ...
    const { data: profiles } = await supabase
      .from('profiles')
      .select('user_id, username, avatar_color, profile_icon, avatar_config, bankroll, lifetime_earnings')
  ...
  async function handleSearch(q) {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('user_id, username, avatar_color, profile_icon, avatar_config')
      .ilike('username', `%${q}%`)
      .not('username', 'is', null)
      .limit(8);
