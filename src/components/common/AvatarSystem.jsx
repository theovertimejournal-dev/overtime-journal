 ...
 *
 * Fixes applied:
 *  - OTJ Bucks deduction for premium items (chains, card backs)
 *  - Error handling on save with user feedback
 *  - skinColor default matches SKIN_TONES array
 *  - <img> onError fallback for CDN rate limits
...
function buildAvatarUrl(config = {}, size = 200) {
  ...
  const fields = [
    'top','hairColor','accessories','accessoriesColor',
    'facialHair','facialHairColor','clothing','clothesColor',
    'clothingGraphic','eyes','eyebrows','mouth','skinColor','style',
  ];
...
const OTJ_CHAINS = {
  ...
  gold_chain: { label: 'Gold Chain',  price: 500,  free: false },
  diamond:    { label: 'Diamond',     price: 800,  free: false },
  otj_logo:   { label: 'OTJ Chain',   price: 1000, free: false },
  cuban:      { label: 'Cuban Link',  price: 700,  free: false },
};

const CARD_BACKS = {
  classic:   { label: 'Classic Red', price: 0,   color: '#c41e3a', free: true },
  flame:     { label: 'Flame',      price: 300, color: '#ff6b35', free: false },
  ice:       { label: 'Ice',        price: 300, color: '#60a5fa', free: false },
  galaxy:    { label: 'Galaxy',     price: 500, color: '#7c3aed', free: false },
  gold_foil: { label: 'Gold Foil',  price: 800, color: '#fbbf24', free: false },
  neon:      { label: 'Neon',       price: 400, color: '#22c55e', free: false },
};

// ─── Default Config ──────────────────────────────────────────────────────────
// FIX: skinColor now uses 'd08b5b' which exists in SKIN_TONES (Medium)
...
const DEFAULT_CONFIG = {
  ...
  mouth: 'smile',
  skinColor: 'd08b5b',       // FIX: was 'd4a574' which didn't match any SKIN_TONES option
  chain: 'none',
  cardBack: 'classic',
};
...
const FALLBACK_SVG = `data:image/svg+xml,${encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><rect fill="%231a1a2e" width="200" height="200" rx="12"/><text x="100" y="110" text-anchor="middle" fill="%234a5568" font-size="48">🎭</text></svg>'
)}`;
...
export function Avatar({ config = {}, size = 120, style = {} }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: 12, overflow: 'hidden', ...style }}>
      <img
        src={url}
        alt="avatar"
        width={size}
...
// ─── Helper: calculate total cost of premium items in a config ───────────────

function calcPremiumCost(config, ownedItems = []) {
  let cost = 0;
  const chainKey = config.chain || 'none';
  const cardKey = config.cardBack || 'classic';

  if (OTJ_CHAINS[chainKey] && !OTJ_CHAINS[chainKey].free && !ownedItems.includes(`chain:${chainKey}`)) {
    cost += OTJ_CHAINS[chainKey].price;
  }
  if (CARD_BACKS[cardKey] && !CARD_BACKS[cardKey].free && !ownedItems.includes(`card:${cardKey}`)) {
    cost += CARD_BACKS[cardKey].price;
  }
  return cost;
}
...
export function AvatarCreator({ userId, currentConfig = {}, onSave, onClose }) {
  ...
  const [ownedItems, setOwnedItems] = useState([]);  // items already purchased
  const [saveError, setSaveError] = useState(null);
  const [saveSuccess, setSaveSuccess] = useState(false);
  const initialConfigRef = useRef({ ...DEFAULT_CONFIG, ...currentConfig });
  const F = "'JetBrains Mono','SF Mono',monospace";

  useEffect(() => {
    if (userId) loadBucksAndOwned();
  }, [userId]);

  async function loadBucksAndOwned() {
    try {
      const { data } = await supabase
        ...
        .single();
      if (data) {
        setBucks(data.bankroll || 0);
      }
    } catch {
      // Non-critical — bucks display will show 0
    }
  }

  // Calculate cost of NEW premium items being equipped (not already owned)
  const purchaseCost = useMemo(() => calcPremiumCost(config, ownedItems), [config, ownedItems]);
  const canAfford = bucks >= purchaseCost;
  ...
  async function handleSave() {
    ...
    try {
      // Save avatar config to profiles
  ...
  const randomize = () => {
    ...
    setConfig({
      ...
      // Randomize keeps chain/cardBack at free defaults to avoid accidental purchases
      chain: 'none',
      cardBack: 'classic',
    });
  };
  ...
  // Check if an item is owned (free items always "owned")
  const isOwned = useCallback((type, key) => {
    if (type === 'chain') return OTJ_CHAINS[key]?.free || ownedItems.includes(`chain:${key}`);
    if (type === 'card') return CARD_BACKS[key]?.free || ownedItems.includes(`card:${key}`);
    return true;
  }, [ownedItems]);
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(255,255,255,0.08)', maxWidth: 520, width: '100%', fontFamily: F,
    }}>
      {/* Header */}
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle at 50% 40%,rgba(196,30,58,0.06) 0%,transparent 60%)',
        borderRadius: 12, marginBottom: 16, border: '1px solid rgba(255,255,255,0.04)',
      }}>
        <img
          src={avatarUrl}
          alt="preview"
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 14, overflowX: 'auto', paddingBottom: 4 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            fontSize: 9, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: F,
          }}>{t.label}</button>
        ))}
      </div>
      ...
      <div style={{
        display: 'grid',
        gridTemplateColumns: isColor ? 'repeat(7,1fr)' : 'repeat(3,1fr)',
        gap: 6, maxHeight: 200, overflowY: 'auto', padding: 2,
      }}>
        {tab === 'hair' && HAIR_STYLES.map(h =>
          <Btn key={h.id} label={h.label} sel={config.top === h.id} onClick={() => update('top', h.id)} />
        )}
        {tab === 'hairColor' && HAIR_COLORS.map(c =>
          <Clr key={c.id} color={c.color} sel={config.hairColor === c.id} onClick={() => update('hairColor', c.id)} />
        )}
        {tab === 'skin' && SKIN_TONES.map(s =>
          <Clr key={s.id} color={s.color} sel={config.skinColor === s.id} onClick={() => update('skinColor', s.id)} />
        )}
        {tab === 'eyes' && EYE_OPTIONS.map(e =>
          <Btn key={e.id} label={e.label} sel={config.eyes === e.id} onClick={() => update('eyes', e.id)} />
        )}
        {tab === 'eyebrows' && EYEBROW_OPTIONS.map(e =>
          <Btn key={e.id} label={e.label} sel={config.eyebrows === e.id} onClick={() => update('eyebrows', e.id)} />
        )}
        {tab === 'mouth' && MOUTH_OPTIONS.map(m =>
          <Btn key={m.id} label={m.label} sel={config.mouth === m.id} onClick={() => update('mouth', m.id)} />
        )}
        {tab === 'facial' && FACIAL_HAIR.map(f =>
          <Btn key={f.id} label={f.label} sel={config.facialHair === f.id} onClick={() => update('facialHair', f.id)} />
        )}
        {tab === 'glasses' && ACCESSORIES_OPTIONS.map(a =>
          <Btn key={a.id} label={a.label} sel={config.accessories === a.id} onClick={() => update('accessories', a.id)} />
        )}
        {tab === 'clothes' && CLOTHING_OPTIONS.map(c =>
          <Btn key={c.id} label={c.label} sel={config.clothing === c.id} onClick={() => update('clothing', c.id)} />
        )}
        {tab === 'clothesColor' && CLOTHES_COLORS.map(c =>
          <Clr key={c.id} color={c.color} sel={config.clothesColor === c.id} onClick={() => update('clothesColor', c.id)} />
        )}
        {tab === 'chain' && Object.entries(OTJ_CHAINS).map(([k, v]) =>
          <Btn
            key={k}
            label={v.label}
            sel={config.chain === k}
            price={v.free ? 0 : v.price}
            owned={isOwned('chain', k)}
            onClick={() => update('chain', k)}
          />
        )}
        {tab === 'cards' && Object.entries(CARD_BACKS).map(([k, v]) => (
          <div key={k} onClick={() => update('cardBack', k)} style={{
            ...
            <div style={{
              width: 30, height: 42, borderRadius: 4, margin: '0 auto 6px',
              background: v.color, border: '2px solid rgba(255,255,255,0.15)',
            }} />
            <div style={{ fontSize: 9, fontWeight: 600, color: config.cardBack === k ? '#f1f5f9' : '#94a3b8' }}>
              {v.label}
            </div>
            {!v.free && (
              <div style={{ fontSize: 8, color: isOwned('card', k) ? '#22c55e' : '#fbbf24', marginTop: 2 }}>
                {isOwned('card', k) ? '✓ OWNED' : `💰 ${v.price}`}
              </div>
            )}
          </div>
        ))}
      ...
      {purchaseCost > 0 && (
        <div style={{
          ...
          background: canAfford ? 'rgba(251,191,36,0.08)' : 'rgba(239,68,68,0.08)',
          border: `1px solid ${canAfford ? 'rgba(251,191,36,0.2)' : 'rgba(239,68,68,0.2)'}`,
          fontSize: 10, color: canAfford ? '#fbbf24' : '#ef4444', textAlign: 'center',
        }}>
          {canAfford
            ? `💰 This will cost ${purchaseCost.toLocaleString()} OTJ Bucks`
            : `❌ Need ${purchaseCost.toLocaleString()} Bucks — you have ${bucks.toLocaleString()}`
          }
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
        <button onClick={randomize} style={{
          ...
          color: '#94a3b8', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: F,
        }}>🎲 RANDOM</button>
        <button
          onClick={handleSave}
          ...
          style={{
            ...
            background: !canAfford ? '#4a5568' : '#c41e3a',
            color: 'white', fontSize: 12, fontWeight: 700,
            cursor: saving || !canAfford ? 'not-allowed' : 'pointer',
            fontFamily: F, letterSpacing: '0.08em',
            opacity: saving ? 0.6 : 1,
          }}
        >
          {saving ? 'SAVING...' : !canAfford ? 'NOT ENOUGH BUCKS' : purchaseCost > 0 ? `SAVE & BUY (${purchaseCost.toLocaleString()})` : 'SAVE AVATAR'}
        </button>
        {onClose && (
...
function Btn({ label, sel, onClick, price, owned }) {
  return (
    <button onClick={onClick} style={{
      ...
      border: sel ? '2px solid #c41e3a' : '2px solid rgba(255,255,255,0.06)',
      fontFamily: "'JetBrains Mono',monospace",
    }}>
      <div style={{ fontSize: 9, fontWeight: 600, color: sel ? '#f1f5f9' : '#94a3b8' }}>{label}</div>
      {price > 0 && (
        <div style={{ fontSize: 8, color: owned ? '#22c55e' : '#fbbf24', marginTop: 2 }}>
          {owned ? '✓ OWNED' : `💰 ${price}`}
        </div>
      )}
    </button>
...
export {
  DEFAULT_CONFIG, HAIR_STYLES, HAIR_COLORS, SKIN_TONES,
  EYE_OPTIONS, EYEBROW_OPTIONS, MOUTH_OPTIONS, FACIAL_HAIR,
  ACCESSORIES_OPTIONS, CLOTHING_OPTIONS, CLOTHES_COLORS,
  OTJ_CHAINS, CARD_BACKS, buildAvatarUrl,
};
