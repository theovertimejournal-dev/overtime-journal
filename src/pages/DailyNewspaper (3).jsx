...
export default function DailyNewspaper() {
  ...
  async function loadNewspaper(date) {
    ...
    const journalQuery = supabase.from("blog_posts")
      .select("*")
      .eq("sport", "nba")
      .order("date", { ascending: false });

    // If viewing a specific date, get that day's post. Otherwise get latest.
    if (selectedDate) {
      journalQuery.eq("date", selectedDate);
    }
  ...
  const viewDateObj = new Date(currentViewDate + 'T12:00:00');
  const volNumber = Math.max(1, Math.floor((viewDateObj - launchDate) / (1000 * 60 * 60 * 24)));

  // Date navigation
  function goToPrevDay() {
    const d = new Date(currentViewDate + 'T12:00:00');
    d.setDate(d.getDate() - 1);
    const prev = d.toLocaleDateString('en-CA');
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Volume / Date line with navigation */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={goToPrevDay} style={{
              ...
              color: '#6b7280', cursor: 'pointer', padding: '2px 8px', fontSize: 11, fontFamily: MONO,
            }}>◀</button>
            <span>{displayDate.toUpperCase()}</span>
            <button onClick={goToNextDay} disabled={isToday} style={{
              background: 'none', border: `1px solid ${isToday ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.1)'}`, borderRadius: 4,
              color: isToday ? '#1e293b' : '#6b7280', cursor: isToday ? 'default' : 'pointer',
              padding: '2px 8px', fontSize: 11, fontFamily: MONO,
            }}>▶</button>
          </div>
